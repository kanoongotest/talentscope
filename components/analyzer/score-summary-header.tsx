/**
 * PURPOSE: Top section of analysis results — score, band badge, "How we score" button, summary
 * INPUTS: Analysis record data, onOpenMethodology callback
 * OUTPUTS: Header card with score, band, candidate info, summary, completion time
...
export function ScoreSummaryHeader({
  ...
  roleType, seniorityLevel, createdAt, modelUsed, latencyMs, promptVersion, onOpenMethodology,
}: Props) {
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-6">
      <div className="flex items-start justify-between">
        ...
        <div className="text-right">
          <p className={`text-5xl font-bold ${scoreColor(scoreTotal)}`}>{Math.round(scoreTotal)}</p>
          <div className="flex items-center gap-1 mt-1 justify-end relative">
            ...
            <button onClick={() => setShowBandInfo(!showBandInfo)} className="text-gray-400 hover:text-gray-600" title="Band info">
              <HelpCircle size={14} />
            </button>
            {showBandInfo && (
              <div className="absolute top-8 right-0 z-10 w-72 rounded-lg border border-gray-200 bg-white p-4 shadow-lg text-left">
                ...
                <div className="space-y-1.5 text-xs text-gray-600">
                  <p><span className="font-semibold text-green-700">A (80-100)</span> Fast-track to interview.</p>
                  <p><span className="font-semibold text-blue-700">B (65-79)</span> Standard interview process.</p>
                  <p><span className="font-semibold text-amber-700">C (50-64)</span> Needs targeted follow-up.</p>
                  <p><span className="font-semibold text-red-700">D (0-49)</span> Below bar.</p>
                </div>
              </div>
            )}
          </div>
          <button onClick={onOpenMethodology} className="text-xs text-[#185CE3] hover:underline mt-1">
            How we score →
          </button>
        </div>
      </div>
